...
void branch_test(BranchUnit *bru) {
    ...
    // If the operation is BNZ and A != 0, the branch pin should contain BRANCH
    // because a movement to a branch must be made.
    if(aluop == ALUOP_BNZ && A != 0)
    {
        pin_set(bru->branch, BRANCH);
    }
    // Else, the branch pin should contain NOBRANCH, since no movement occurs.
    else
    {
