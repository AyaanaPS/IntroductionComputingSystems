...
find_space_loop:
        ...
        # (EDITED)
        jnc     find_space_continue       # Jump if no carry to find_space_cont
        add     $256, %ebx                # If there is carry, account for it
                                          #     by adding 256 to %ebx.


find_space_continue:
        add     $2, %esi                  # Move to the next count!(EDITED)
...
find_space_done:
        ...
        # (EDITED)
        push    %ecx              # Store the value of the caller-save reg
        push    %ebx              # Number of bytes to allocate...
        call    malloc
        add     $4, %esp          # Clean up stack after call.
        pop     %ecx              # Restore the value of the caller-save reg
