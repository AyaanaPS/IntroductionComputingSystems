...
float Shape_getMass(Shape_Data *this) {
    ...
    float mass;
    mass = this->class->getVolume(this) * this->density;
    return mass;
}
...
Box_Data * new_Box(float L, float W, float H, float D) {
    ...
    Box_Class * c = malloc(sizeof(Box_Class));
    Box_class_init(c);
    Box_Data * d = malloc(sizeof(Box_Data));
    Box_init(d, c, L, W, H, D);
...
Sphere_Data * new_Sphere(float R, float D) {
    ...
    Sphere_Class * c = malloc(sizeof(Sphere_Class));
    Sphere_class_init(c);
    Sphere_Data * d = malloc(sizeof(Sphere_Data));
    Sphere_init(d, c, R, D);
    return d;
}
...
float Sphere_getVolume(Sphere_Data *this) {
    ...
    float volume;
    volume = (4.0 / 3.0) * 3.14159 * this->radius * this->radius * this->radius; 
    return volume;
}
...
Cone_Data * new_Cone(float BR, float H, float D) {
    ...
    Cone_Class * c = malloc(sizeof(Cone_Class));
    Cone_class_init(c);
    Cone_Data * d = malloc(sizeof(Cone_Data));
    Cone_init(d, c, BR, H, D);
    return d;
}
...
float Cone_getVolume(Cone_Data *this) {
    ...
    float volume;
    volume = (1.0 / 3.0) * 3.14159 * this->base_radius * this->base_radius * this->height;
    return volume;
}
