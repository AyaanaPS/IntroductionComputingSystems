...
typedef struct multimap_value {
    /* This stores an array of int values associated with the key */
    int value[ARRAY_SIZE];

    /* This is the currently occupied part of the array */
    int currentSize;

    /* This is the next value struct */
...
void mm_add_value(multimap *mm, int key, int value) {
    ...
    /* If there are no values, we have to create a value struct for the node */
    if(node->values == NULL)
    {   
    ...
    {
        /* We have to find the array to put the value into */
        lastArr = node->values_tail;
        /* If the last array of the node is full we have to make a new one */
        ...
        {
            /* Else, we can just add the value to the last array */
...
int mm_contains_pair(multimap *mm, int key, int value) {
    ...
    /* Look through all the values of the key */
    curr = node->values;
    while (curr != NULL) {
