...
void sthread_start(int timer) {
    ...
    /* We add a lock here since we want to lock all entry points to the
     * scheduler and the __sthread_start() code in glue.s calls the 
     * scheduler. We want to lock before calling the scheduler because 
     * the scheduler modifies the queue and we don't want anything else to
     * to change the queue while this call is changing it.
     */
    __sthread_lock();

    __sthread_start();
    /* We don't have to unlock after __sthread_start() since __sthread_schedule
     * unlocks at the end of function.
     */
}
 ...
 */
void __sthread_finish(void) {
    /* We lock here because this function calls __sthread_schedule() and we
     * don't want to let anything else call schedule while this one is calling
     * it.
     */
    __sthread_lock();
...
void sthread_block(int lock) {
    ...
    /* This checks to see if the thread has already been locked before calling
     * this function. If it has, we don't want to double lock it.
     */
    if(lock)
    {
        /* We lock here because this function calls __sthread_schedule() and we
...
void sthread_unblock(Thread *threadp, int lock) {
    ...
    if(lock)
    {
        /* We lock here because this function modifies the queue and we don't
         * want to let any other thread modify the queue while this one 
         * is changing it.
         */
        __sthread_lock();
    }
    ...
    /* We unlock here so that other threads can modify the queue now. */
    __sthread_unlock();
}